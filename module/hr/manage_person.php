    ...
    <div class="row">
        <div class="col-sm-12" style="padding:0 25px 0 30px;">
            <div class="panel panel-default">
                ...
                $count_not_resign = 0;
                $count_resign = 0;  
                $sql_select_person_normal = "
                    SELECT person_id FROM hr_person 
                    WHERE deleted!=1  
                ";
                $query_select_person = mysqli_query($conn, $sql_select_person_normal);
                while ($assoc_select_person = mysqli_fetch_assoc($query_select_person)) {
                    $sql_select_person_notleave = "
                      SELECT person_id,employment_start_break,employment_finish_break FROM hr_person_employment
                      WHERE person_id='{$assoc_select_person["person_id"]}' AND employment_work_status='1'
                    ";
                    $sql_select_person_leave = "
                      SELECT person_id,employment_start_break,employment_finish_break FROM hr_person_employment
                      WHERE person_id='{$assoc_select_person["person_id"]}' AND employment_work_status='2'
                    ";
                    $query_select_person_notleave = mysqli_query($conn, $sql_select_person_notleave);
                    $query_select_person_leave = mysqli_query($conn, $sql_select_person_leave);

                    while ($assoc_notleave = mysqli_fetch_assoc($query_select_person_notleave)) {
                        if($assoc_notleave["employment_start_break"]!="" AND $assoc_notleave["employment_finish_break"]!=""){

                            if($assoc_notleave["employment_finish_break"]==$present_date){
                                $sql_update_break_status="
                                    UPDATE hr_person_employment SET 
                                    employment_break_status='1',
                                    employment_start_break=NULL,
                                    employment_finish_break=NULL,
                                    employment_reason_break=NULL
                                    WHERE person_id='{$assoc_notleave["person_id"]}'
                                ";
                                $query_update_break_status = mysqli_query($conn,$sql_update_break_status)or die("Sql update break status error : ".mysqli_error($conn));
                            }
                        }
                        $count_not_resign += 1;
                    }

                    while ($assoc_leave = mysqli_fetch_assoc($query_select_person_leave)) {
                        $count_resign += 1;
                    }
                }
                ?>
                    ...
                    <div class="table-responsive">
                        <div class="col-sm-12" style="padding:10px 15px 5px 0; text-align:right;">
                            <button type="button" class="btn btn-success btn-sm"
                                ...
                                <span class='badge'><?php echo($count_not_resign!=0)?"$count_not_resign":"0"; ?></span> พนักงานทั่วไป
                            </button>
                            <button type="button" class="btn btn-danger btn-sm"
                                    onclick="window.location.href='index.php?mod=hr/manage_person_resign';">
                                <span class='badge'><?php echo($count_resign!=0)?"$count_resign":"0"; ?></span> พนักงาน ลาออก
                            </button>
